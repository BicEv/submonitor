...
public class SubscriptionRestControllerIT {
    ...
    private SubCreationRequest invalidIdCreationRequest = new SubCreationRequest(9999990L, new BigDecimal(100), "RUB",
    ...
    void setUp() {
        ...
        invalidDataCreationRequest = new SubCreationRequest(serviceId, new BigDecimal(0), "USD", BillingPeriod.MONTHLY,
                LocalDate.now().minusDays(7));
        updateRequest = new SubUpdateRequest(new BigDecimal(15.99), "USD", null, null, null);
        subscription = subscriptionRepository
                .save(Subscription.builder().billingPeriod(BillingPeriod.YEARLY).price(new BigDecimal(100))
                        .currency("USD").isActive(true).isDeleted(false).nextPayment(LocalDate.now().plusMonths(6))
                        .service(service).subscriber(owner).build());
        subscriptionId = subscription.getId();

        for (int i = 0; i < 4; i++) {
            subscriptions.add(subscriptionRepository
                    .save(Subscription.builder().billingPeriod(BillingPeriod.YEARLY).price(new BigDecimal((1 + i) * 50))
                            .currency("USD").isActive(i == 3 ? false : true).isDeleted(i == 3 ? true : false)
